...
Download `prebuilt/lua` from this repo, copy it to your SCO box, run it.

```bash
# from a machine that can ssh to the SCO box:
scp prebuilt/lua root@your-sco-host:/tmp/lua
ssh root@your-sco-host 'chmod +x /tmp/lua && /tmp/lua -v'
```

Or directly on the SCO box if you have web/ftp access:

```sh
# on the SCO box itself:
chmod +x lua
./lua -v
```
...
For a permanent install, drop it somewhere on `PATH`, e.g.:

```sh
cp lua /usr/local/bin/lua
```
